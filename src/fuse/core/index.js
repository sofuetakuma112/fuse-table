...
export default class Fuse {
  ...
  setCollection(docs, index) {
    ...
    // default
    // this.options.fieldNormWeight = 1
    this._myIndex =
      index ||
  ...
  search(query, { limit = -1 } = {}) {
    ...
    if (isString(query)) {
      if (isString(this._docs[0])) {
        // docsがstring[]
        results = this._searchStringList(query);
      } else {
        // docsが{}[]
        results = this._searchObjectList(query);
      }
    } else {
      // queryがstring以外
  ...
  _searchObjectList(query) {
    ...
    // records:
    //   { i: docIndex, $: { [number]: { v: value, n: this.norm.get(value) } } }[]
    // (検索対象の値の配列?)
